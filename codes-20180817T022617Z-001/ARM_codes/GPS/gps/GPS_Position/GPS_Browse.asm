...
SUB	SP, SP, #4
STR	LR, [SP, #0]
;GPS_Browse.c,58 :: 		string = strstr(txt,"$GPGGA");
...
STR	R0, [R1, #0]
;GPS_Browse.c,59 :: 		if(string != 0) {                      // If txt array contains "$GPGLL" string we proceed...
CMP	R0, #0
IT	EQ
BEQ	L_GPS_messagePARSER2
;GPS_Browse.c,60 :: 		if(string[43] == '1') {               // if "$GPGGA"          message have '1' sign in the 43-th
...
;GPS_Browse.c,63 :: 		latitude = (string[18]-48)*10 + (string[19]-48);
...
STRH	R1, [R0, #0]
;GPS_Browse.c,64 :: 		longitude = (string[30]-48)*100 + (string[31]-48)*10 + (string[32]-48);
MOV	R0, R3
LDR	R0, [R0, #0]
ADDS	R0, #30
LDRB	R0, [R0, #0]
...
STRH	R1, [R0, #0]
;GPS_Browse.c,65 :: 		if(string[28] == 'S') {            // if the latitude is in the South direction it has minus sign
MOV	R0, R3
LDR	R0, [R0, #0]
ADDS	R0, #28
LDRB	R0, [R0, #0]
...
;GPS_Browse.c,66 :: 		latitude = 0 - latitude;
...
;GPS_Browse.c,68 :: 		if(string[41] == 'W') {            // if the longitude is in the West direction it has minus sign
MOVW	R0, #lo_addr(_string+0)
MOVT	R0, #hi_addr(_string+0)
LDR	R0, [R0, #0]
ADDS	R0, #41
...
;GPS_Browse.c,69 :: 		longitude = 0 - longitude;
...
;GPS_Browse.c,72 :: 		lat_str[0] = string[18];
...
STRB	R1, [R0, #0]
;GPS_Browse.c,73 :: 		lat_str[1] = string[19];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #19
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;GPS_Browse.c,74 :: 		lat_str[2] = ',';
MOVS	R1, #44
MOVW	R0, #lo_addr(_lat_str+2)
MOVT	R0, #hi_addr(_lat_str+2)
STRB	R1, [R0, #0]
;GPS_Browse.c,75 :: 		lat_str[3] = string[20];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #20
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;GPS_Browse.c,76 :: 		lat_str[4] = string[21];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #21
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;GPS_Browse.c,77 :: 		lat_str[5] = ' ';
MOVS	R1, #32
MOVW	R0, #lo_addr(_lat_str+5)
MOVT	R0, #hi_addr(_lat_str+5)
STRB	R1, [R0, #0]
;GPS_Browse.c,78 :: 		lat_str[6] = string[28];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #28
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;GPS_Browse.c,79 :: 		lat_str[7] = 0;
MOVS	R1, #0
MOVW	R0, #lo_addr(_lat_str+7)
MOVT	R0, #hi_addr(_lat_str+7)
STRB	R1, [R0, #0]
;GPS_Browse.c,81 :: 		long_str[0] = string[30];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #30
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;GPS_Browse.c,82 :: 		long_str[1] = string[31];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #31
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;GPS_Browse.c,83 :: 		long_str[2] = string[32];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #32
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;GPS_Browse.c,84 :: 		long_str[3] = ',';
MOVS	R1, #44
MOVW	R0, #lo_addr(_long_str+3)
MOVT	R0, #hi_addr(_long_str+3)
STRB	R1, [R0, #0]
;GPS_Browse.c,85 :: 		long_str[4] = string[33];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #33
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;GPS_Browse.c,86 :: 		long_str[5] = string[34];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #34
LDRB	R1, [R0, #0]
...
STRB	R1, [R0, #0]
;GPS_Browse.c,87 :: 		long_str[6] = ' ';
MOVS	R1, #32
MOVW	R0, #lo_addr(_long_str+6)
MOVT	R0, #hi_addr(_long_str+6)
STRB	R1, [R0, #0]
;GPS_Browse.c,88 :: 		long_str[7] = string[41];
MOV	R0, R2
LDR	R0, [R0, #0]
ADDS	R0, #41
LDRB	R1, [R0, #0]
...
_main:
;GPS_Browse.c,93 :: 		void main(void){
SUB	SP, SP, #8
;GPS_Browse.c,95 :: 		TFT_Set_Default_Mode();
BL	_TFT_Set_Default_Mode+0
;GPS_Browse.c,96 :: 		TFT_Init_ILI9341_8bit(320, 240);
MOVS	R1, #240
MOVW	R0, #320
BL	_TFT_Init_ILI9341_8bit+0
;GPS_Browse.c,97 :: 		TFT_Fill_Screen(CL_BLACK);
MOVW	R0, #0
BL	_TFT_Fill_Screen+0
...
NOP
NOP
;GPS_Browse.c,101 :: 		GPIO_Digital_Output(&GPIOC_ODR,_GPIO_PINMASK_2);
MOVW	R1, #4
MOVW	R0, #lo_addr(GPIOC_ODR+0)
MOVT	R0, #hi_addr(GPIOC_ODR+0)
BL	_GPIO_Digital_Output+0
;GPS_Browse.c,102 :: 		GPIOC_ODR=0;
MOVS	R1, #0
MOVW	R0, #lo_addr(GPIOC_ODR+0)
MOVT	R0, #hi_addr(GPIOC_ODR+0)
STR	R1, [R0, #0]
;GPS_Browse.c,103 :: 		Delay_ms(100);
MOVW	R7, #9043
MOVT	R7, #8
NOP
NOP
...
NOP
NOP
;GPS_Browse.c,104 :: 		GPIOC_ODR = 4;
MOVS	R1, #4
MOVW	R0, #lo_addr(GPIOC_ODR+0)
MOVT	R0, #hi_addr(GPIOC_ODR+0)
STR	R1, [R0, #0]
;GPS_Browse.c,105 :: 		Display_Init();
BL	_Display_Init+0
;GPS_Browse.c,106 :: 		while(1){
L_main10:
;GPS_Browse.c,107 :: 		for(counter=0;counter<768;counter++)
MOVS	R0, #0
SXTH	R0, R0
STRH	R0, [SP, #0]
L_main12:
...
IT	GE
BGE	L_main13
;GPS_Browse.c,109 :: 		txt[counter]=UART3_Read();
LDRSH	R1, [SP, #0]
MOVW	R0, #lo_addr(_txt+0)
MOVT	R0, #hi_addr(_txt+0)
ADDS	R0, R0, R1
...
BAL	L_main12
L_main13:
;GPS_Browse.c,111 :: 		GPS_messagePARSER();
BL	_GPS_messagePARSER+0
;GPS_Browse.c,112 :: 		Display_Cursor(latitude, longitude, lat_str, long_Str,CL_Yellow);
MOVW	R2, #65504
MOVW	R0, #lo_addr(_longitude+0)
MOVT	R0, #hi_addr(_longitude+0)
LDRSH	R1, [R0, #0]
